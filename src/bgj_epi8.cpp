...
static int bgj_epi8_env_flag(const char *name, int default_value)
{
...
static uint64_t bgj_epi8_env_u64(const char *name, uint64_t default_value)
{
    const char *env = getenv(name);
...
{
    const char *env = getenv("BGJ_CUDA_WORK_SORT");
    if (env && env[0]) return env[0] != '0';
    return 0;
}

static int bgj_epi8_cuda_search0_staged_default()
{
...
{
    if (!bucket) return 0;
    return bgj_epi8_bucket_pair_dots(bucket->num_pvec, bucket->num_nvec);
}

template <bool record_dp>
static void bgj_epi8_cuda_sort_buckets_by_work(bucket_epi8_t<record_dp> **buckets,
                                               long num_bucket)
{
    ...
    std::sort(buckets, buckets + num_bucket,
              [](const bucket_epi8_t<record_dp> *a,
...
{
    uint32_t value = bgj_cuda_batch_size(search_threads);
    const char *batch_env = getenv("BGJ_CUDA_BGJ2_BATCH");
    const uint64_t size_override = bgj_epi8_env_u64("BGJ_CUDA_BGJ2_BATCH_SIZE", 0);

    if (batch_env && batch_env[0] && batch_env[0] == '0') {
        value = 1;
    } else if (size_override > 0) {
...
{
    uint32_t value = 1;
    const char *batch_env = getenv("BGJ_CUDA_BGJ3_BATCH");
    const char *np_fused_env = getenv("BGJ_CUDA_BGJ3_SEARCH2_NP_FUSED");
    const char *full_fused_env = getenv("BGJ_CUDA_BGJ3_SEARCH2_FULL_FUSED");
    const uint64_t size_override = bgj_epi8_env_u64("BGJ_CUDA_BGJ3_BATCH_SIZE", 0);

    if (batch_env && batch_env[0] && batch_env[0] == '0') {
        value = 1;
    } else if (size_override > 0) {
        value = size_override > BGJ3_EPI8_BUCKET2_BATCHSIZE ?
                BGJ3_EPI8_BUCKET2_BATCHSIZE : (uint32_t)size_override;
    } else if (batch_env && batch_env[0]) {
        value = 8;
    ...
    if (value < 1) value = 1;
    if (value > BGJ3_EPI8_BUCKET2_BATCHSIZE) value = BGJ3_EPI8_BUCKET2_BATCHSIZE;
    return value;
}

static int bgj3_epi8_cuda_search2_staged_requested()
{
    const char *env = getenv("BGJ_CUDA_BGJ3_SEARCH2_STAGED");
    return env && env[0] && env[0] != '0';
}

static int bgj3_epi8_cuda_search2_np_fused_requested()
{
    const char *env = getenv("BGJ_CUDA_BGJ3_SEARCH2_NP_FUSED");
    return env && env[0] && env[0] != '0';
}
...
{
    const uint32_t global_capacity = bgj_cuda_result_capacity();
    const uint64_t override_capacity =
        bgj_epi8_env_u64("BGJ_CUDA_BGJ3_SEARCH2_NP_FUSED_MAX_RESULTS", 0);
    uint32_t capacity = global_capacity;

    if (override_capacity > 0) {
        capacity = override_capacity > 0xffffffffULL ?
                   0xffffffffu : (uint32_t)override_capacity;
    } else if (capacity > (1u << 18)) {
        capacity = 1u << 18;
    }
    if (capacity == 0) capacity = global_capacity;
    return capacity;
}

static int bgj3_epi8_cuda_search2_cpu_profile_requested()
{
    const char *env = getenv("BGJ_CUDA_BGJ3_SEARCH2_CPU_PROFILE");
    return env && env[0] && env[0] != '0';
}
#endif
...
#if defined(HAVE_CUDA)
               || bgj_epi8_parse_env_double("BGJ_CUDA_BUCKET_TARGET_SIZE", &value)
...
void bgj_profile_data_t<nb>::search1_log() {
    if (log_level >= 3 && p->CSD > MIN_LOG_CSD) {
        ...
        fprintf(log_out, "try_add2 = %ld, try_add3 = %ld, succ_add2 = %ld(1/%.2f), succ_add3 = %ld(1/%.2f)\n",
                try_add2, try_add3, succ_add2, (try_add2+1e-20)/succ_add2, succ_add3, (try_add3+1e-20)/succ_add3);
        if (cuda_single_bucket1 || cuda_batch_bucket1 || cuda_fallback_bucket1) {
            fprintf(log_out, "cuda search1: single=%lu buckets/%lu dp in %.6fs, batch=%lu calls/%lu buckets/%lu dp in %.6fs, cred=%.6fs, fallback=%lu buckets/%lu dp in %.6fs\n",
                    cuda_single_bucket1, cuda_single_ndp1, cuda_single_time1,
                    cuda_batch_call1, cuda_batch_bucket1, cuda_batch_ndp1, cuda_batch_time1,
                    cuda_cred_time1, cuda_fallback_bucket1, cuda_fallback_ndp1, cuda_fallback_time1);
        }
    }
}
    ...
    if (log_level >= 1 && p->CSD > MIN_LOG_CSD) {
        ...
        if (bgj == 3) {
            ...
            if (log_level >= 2) {
                ...
                if (cuda_single_bucket2 || cuda_batch_bucket2 || cuda_fallback_bucket2) {
                    fprintf(log_out, "cuda search2: single=%lu buckets/%lu dp in %.6fs, batch=%lu calls/%lu buckets/%lu dp in %.6fs, cred=%.6fs, fallback=%lu buckets/%lu dp in %.6fs\n",
                            cuda_single_bucket2, cuda_single_ndp2, cuda_single_time2,
                            cuda_batch_call2, cuda_batch_bucket2, cuda_batch_ndp2, cuda_batch_time2,
                            cuda_cred_time2, cuda_fallback_bucket2, cuda_fallback_ndp2, cuda_fallback_time2);
                }
            }
        }
    }
...
void bgj_profile_data_t<nb>::final_log(int bgj, long sieving_stucked) {
    ...
    if (bgj == 3 && bgj_epi8_bgj3_cuda_profile_requested() && p->CSD > MIN_LOG_CSD) {
        fprintf(log_err,
                ...
                "cuda2_reasons=min_dot:%lu/%lu,min_batch:%lu/%lu,batch_fail:%lu/%lu,consume_fail:%lu/%lu,overflow:%lu/%lu "
                "cuda2_cpu=buckets:%lu,ndp:%lu,cred:%.6fs,np:%.6fs,pp:%.6fs,nn:%.6fs "
                "try_add2=%lu try_add3=%lu succ_add2=%lu succ_add3=%lu\n",
                p->CSD,
                bucket0_time,
                bucket1_time,
                ...
                cuda_fallback_time2,
                (unsigned long)cuda2_reason_min_dot_bucket,
                (unsigned long)cuda2_reason_min_dot_ndp,
                (unsigned long)cuda2_reason_min_batch_bucket,
                (unsigned long)cuda2_reason_min_batch_ndp,
                (unsigned long)cuda2_reason_batch_fail_bucket,
                (unsigned long)cuda2_reason_batch_fail_ndp,
                (unsigned long)cuda2_reason_consume_fail_bucket,
                (unsigned long)cuda2_reason_consume_fail_ndp,
                (unsigned long)cuda2_overflow_bucket,
                (unsigned long)cuda2_overflow_ndp,
                (unsigned long)cuda2_cpu_bucket,
                (unsigned long)cuda2_cpu_ndp,
                cuda2_cpu_cred_time,
                cuda2_cpu_np_time,
                cuda2_cpu_pp_time,
                cuda2_cpu_nn_time,
...
void bgj_profile_data_t<nb>::combine(bgj_profile_data_t<nb> *prof) {
    ...
    cuda_cred_time1 += prof->cuda_cred_time1;
    cuda_single_time1 += prof->cuda_single_time1;
    cuda_batch_time1 += prof->cuda_batch_time1;
    cuda_fallback_time1 += prof->cuda_fallback_time1;
    cuda_cred_time2 += prof->cuda_cred_time2;
    cuda_single_time2 += prof->cuda_single_time2;
    cuda_batch_time2 += prof->cuda_batch_time2;
    cuda_fallback_time2 += prof->cuda_fallback_time2;
    ...
    cuda_single_ndp1 += prof->cuda_single_ndp1;
    cuda_batch_ndp1 += prof->cuda_batch_ndp1;
    cuda_fallback_ndp1 += prof->cuda_fallback_ndp1;
    cuda_single_bucket1 += prof->cuda_single_bucket1;
    cuda_batch_bucket1 += prof->cuda_batch_bucket1;
    cuda_batch_call1 += prof->cuda_batch_call1;
    cuda_fallback_bucket1 += prof->cuda_fallback_bucket1;
    cuda_single_ndp2 += prof->cuda_single_ndp2;
    cuda_batch_ndp2 += prof->cuda_batch_ndp2;
    cuda_fallback_ndp2 += prof->cuda_fallback_ndp2;
    cuda_single_bucket2 += prof->cuda_single_bucket2;
    cuda_batch_bucket2 += prof->cuda_batch_bucket2;
    cuda_batch_call2 += prof->cuda_batch_call2;
    cuda_fallback_bucket2 += prof->cuda_fallback_bucket2;
    cuda2_reason_min_dot_bucket += prof->cuda2_reason_min_dot_bucket;
    cuda2_reason_min_dot_ndp += prof->cuda2_reason_min_dot_ndp;
    cuda2_reason_min_batch_bucket += prof->cuda2_reason_min_batch_bucket;
    cuda2_reason_min_batch_ndp += prof->cuda2_reason_min_batch_ndp;
    cuda2_reason_batch_fail_bucket += prof->cuda2_reason_batch_fail_bucket;
    cuda2_reason_batch_fail_ndp += prof->cuda2_reason_batch_fail_ndp;
    cuda2_reason_consume_fail_bucket += prof->cuda2_reason_consume_fail_bucket;
    cuda2_reason_consume_fail_ndp += prof->cuda2_reason_consume_fail_ndp;
    cuda2_overflow_bucket += prof->cuda2_overflow_bucket;
    cuda2_overflow_ndp += prof->cuda2_overflow_ndp;
    cuda2_cpu_bucket += prof->cuda2_cpu_bucket;
    cuda2_cpu_ndp += prof->cuda2_cpu_ndp;
    cuda2_cpu_cred_time += prof->cuda2_cpu_cred_time;
    cuda2_cpu_np_time += prof->cuda2_cpu_np_time;
    cuda2_cpu_pp_time += prof->cuda2_cpu_pp_time;
    cuda2_cpu_nn_time += prof->cuda2_cpu_nn_time;
...
int Pool_epi8_t<nb>::bgj1_Sieve(long log_level, long lps_auto_adj){
    ...
    while (!sieve_is_over(saturation_radius, saturation_ratio) && !sieving_stucked) {
        ...
        do {
            ...
            #if defined(HAVE_CUDA)
            bgj_epi8_cuda_sort_buckets_by_work<BGJ1_EPI8_USE_3RED>(main_bucket, BGJ1_EPI8_BUCKET_BATCHSIZE);
            #endif
            ...
            for (long thread = 0; thread < search_threads; thread++){
                ...
                while (nrem_bucket) {
                    ...
                    #if defined(HAVE_CUDA)
                    if (bgj_cuda_search_requested()) {
                        const int cuda_cred_on_device = bgj_cuda_cred_transform_requested();
                        const int cuda_overlap_cred = !cuda_cred_on_device && bgj_cuda_overlap_cred_requested();
                        if (cuda_batch_capacity > 1 &&
                            ...
                            if (cuda_num_bucket >= (long)cuda_min_batch) {
                                ...
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    ...
                                    bool one_cred_done = false;
                                    bool one_cuda_done = false;
                                    if (ndp >= cuda_batch_min_dots) {
                                        double t0 = 0.0;
                                        if (cuda_overlap_cred) {
                                            t0 = bgj_epi8_wall_time();
                                            one_cuda_done =
                                                _search_bgj1_cuda_overlap<BGJ1_EPI8_USE_3RED, 1>(cuda_batch[i],
                                                                                                 sol_list[thread],
                                                                                                 goal_norm,
                                                                                                 &local_profile) > 0;
                                    ...
                                    if (!one_cuda_done) {
                                        if (!one_cred_done) {
                                            const double cred_t0 = bgj_epi8_wall_time();
                                            _search_cred<BGJ1_EPI8_USE_3RED, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                            __cuda_cred_time0 += bgj_epi8_wall_time() - cred_t0;
                                        }
                        ...
                        if (ndp >= cuda_batch_min_dots) {
                            ...
                            if (cuda_overlap_cred) {
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda_overlap<BGJ1_EPI8_USE_3RED, 1>(bkt, sol_list[thread], goal_norm, &local_profile) > 0;
                                __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                                cred_done = true;
                            } else {
                                if (!cuda_cred_on_device) {
...
int Pool_epi8_t<nb>::bgj2_Sieve(long log_level, long lps_auto_adj){
    ...
    while (!sieve_is_over(saturation_radius, saturation_ratio) && !sieving_stucked) {
        ...
        do {
            ...
            #if defined(HAVE_CUDA)
            bgj_epi8_cuda_sort_buckets_by_work<BGJ2_EPI8_REUSE_USE_3RED>(rbucket, BGJ2_EPI8_BUCKET0_BATCHSIZE);
            #endif
            ...
            local_profile.sol_check(sol_list, num_threads, goal_norm, uid);
            #if defined(HAVE_CUDA)
            bgj_epi8_cuda_sort_buckets_by_work<0>(bucket1, BGJ2_EPI8_BUCKET0_BATCHSIZE * BGJ2_EPI8_BUCKET1_BATCHSIZE);
            #endif
            ...
            const int cuda_bgj2_search0_ordered =
                cuda_bgj2_search0 ? bgj_epi8_env_flag("BGJ_CUDA_BGJ2_SEARCH0_ORDERED",
                                                       0) : 0;
            const int cuda_bgj2_search0_staged =
                cuda_bgj2_search0 ? bgj_epi8_env_flag("BGJ_CUDA_BGJ2_SEARCH0_STAGED",
                                                       bgj_epi8_cuda_search0_staged_default()) : 0;
            #endif

            ///////////////// search0 /////////////////
            #if defined(HAVE_CUDA)
            if (cuda_bgj2_search0_staged && bgj_cuda_search_requested() && cuda_bgj2_search0) {
                TIMER_START;
                struct bgj2_cuda_search0_staged_task_t {
                    bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *buckets[BGJ2_EPI8_BUCKET0_BATCHSIZE];
                    long num_bucket = 0;
                    long owner = 0;
                    int use_cuda = 0;
                    int cuda_done = 0;
                    ...
                    double cred_sec = 0.0;
                    double cuda_sec = 0.0;
                    std::unique_ptr<bgj_cuda_result_t[]> results;
                    std::vector<bgj_cuda_result_t *> result_ptrs;
                    std::vector<uint32_t> result_counts;
                    std::vector<int> overflows;
                };

                std::vector<bgj2_cuda_search0_staged_task_t> staged_tasks;
                int staged_build_ok = 1;
                try {
                    staged_tasks.reserve(BGJ2_EPI8_BUCKET0_BATCHSIZE);
                } catch (...) {
                    staged_build_ok = 0;
                }

                auto staged_finish_task =
                    [&](bgj2_cuda_search0_staged_task_t &task) {
                        task.batch_ndp_long = 0;
                        task.batch_ndp = 0;
                        for (long i = 0; i < task.num_bucket; i++) {
                            const long ndp = bgj_epi8_search0_profile_ndp(task.buckets[i]->num_pvec,
                                                                          task.buckets[i]->num_nvec);
                            task.batch_ndp_long += ndp;
                            if (ndp > 0) task.batch_ndp += (uint64_t)ndp;
                        }
                    };

                auto staged_push_single =
                    [&](bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *bkt) {
                        bgj2_cuda_search0_staged_task_t task;
                        task.buckets[0] = bkt;
                        task.num_bucket = 1;
                        task.use_cuda =
                            bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots;
                        staged_finish_task(task);
                        staged_tasks.push_back(std::move(task));
                    };

                if (staged_build_ok) {
                    long nrem_rbucket = BGJ2_EPI8_BUCKET0_BATCHSIZE;
                    while (nrem_rbucket > 0) {
                        if (local_profile.succ_add2 + local_profile.succ_add3 >
                            num_empty + sorted_index - goal_index) {
                        ...
                        bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *bkt = rbucket[nrem_rbucket - 1];
                        nrem_rbucket--;
                        if (!bkt) continue;
                        ...
                        if (cuda_batch_capacity > 1 &&
                            ...
                            while (task.num_bucket < (long)cuda_batch_capacity &&
                                   task.num_bucket < BGJ2_EPI8_BUCKET0_BATCHSIZE &&
                                   nrem_rbucket > 0) {
                                if (local_profile.succ_add2 + local_profile.succ_add3 >
                                    num_empty + sorted_index - goal_index) {
                                    local_profile.report_bucket_not_used(2, nrem_rbucket, 0);
                                ...
                                bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *next = rbucket[nrem_rbucket - 1];
                                nrem_rbucket--;
                                if (next) task.buckets[task.num_bucket++] = next;
                            }

                            if (task.num_bucket >= (long)cuda_min_batch) {
                                staged_finish_task(task);
                                staged_tasks.push_back(std::move(task));
                            } else {
                                for (long i = 0; i < task.num_bucket; i++) staged_push_single(task.buckets[i]);
                            }
                        } else {
                            staged_push_single(bkt);
                        }
                    }
                }

                const uint32_t staged_result_capacity = bgj_cuda_result_capacity();
                const int cuda_cred_on_device = bgj_cuda_cred_transform_requested();

                if (staged_build_ok && staged_result_capacity != 0) {
                    #pragma omp parallel for schedule(dynamic)
                    for (long task_ind = 0; task_ind < (long)staged_tasks.size(); task_ind++) {
                        bgj2_cuda_search0_staged_task_t &task = staged_tasks[task_ind];
                        const long owner = omp_get_thread_num();
                        task.owner = owner;
                        if (!task.use_cuda) continue;

                        if (!cuda_cred_on_device) {
                            const double t0 = bgj_epi8_wall_time();
                            for (long i = 0; i < task.num_bucket; i++) {
                                _search_cred<BGJ2_EPI8_REUSE_USE_3RED, 1>(task.buckets[i],
                                                                          sol_list[owner],
                                                                          goal_norm,
                                                                          &local_profile);
                            }
                            task.cred_sec += bgj_epi8_wall_time() - t0;
                            task.cred_done = 1;
                        }
                        ...
                        try {
                            const size_t total_capacity =
                                (size_t)task.num_bucket * (size_t)staged_result_capacity;
                            task.results.reset(new bgj_cuda_result_t[total_capacity]);
                            task.result_ptrs.resize((size_t)task.num_bucket);
                            task.result_counts.assign((size_t)task.num_bucket, 0);
                            task.overflows.assign((size_t)task.num_bucket, 0);
                        } catch (...) {
                            storage_ok = 0;
                        }
                        if (!storage_ok) continue;

                        const double t0 = bgj_epi8_wall_time();
                        task.cuda_done =
                            _search_bgj1_cuda_batch_collect<BGJ2_EPI8_REUSE_USE_3RED, 1>(
                                task.buckets,
                                task.num_bucket,
                                goal_norm,
                                task.results.get(),
                                staged_result_capacity,
                                task.result_ptrs.data(),
                                task.result_counts.data(),
                                task.overflows.data()) > 0;
                        task.cuda_sec += bgj_epi8_wall_time() - t0;
                    }
                }
                ...
                for (long task_ind = 0; task_ind < (long)staged_tasks.size(); task_ind++) {
                    bgj2_cuda_search0_staged_task_t &task = staged_tasks[task_ind];
                    const long owner = (task.owner >= 0 && task.owner < num_threads) ? task.owner : 0;
                    __search0_ndp += task.batch_ndp_long;
                    __cuda_cred_time0 += task.cred_sec;

                    if (task.use_cuda) {
                        if (task.num_bucket > 1) {
                            __cuda_batch_time0 += task.cuda_sec;
                            __cuda_batch_call0++;
                        } else {
                            __cuda_single_time0 += task.cuda_sec;
                        }
                    }
                    ...
                    if (task.cuda_done) {
                        int consume_ok = 1;
                        for (long i = 0; i < task.num_bucket; i++) {
                            if (!_consume_bgj1_cuda_results<BGJ2_EPI8_REUSE_USE_3RED, 1>(
                                    task.buckets[i],
                                    sol_list[owner],
                                    &local_profile,
                                    task.result_ptrs[i],
                                    task.result_counts[i])) {
                                consume_ok = 0;
                                break;
                            }
                        }
                        if (consume_ok) {
                            if (task.num_bucket > 1) {
                                __cuda_batch_bucket0 += (uint64_t)task.num_bucket;
                                __cuda_batch_ndp0 += task.batch_ndp;
                            } else {
                                __cuda_single_bucket0++;
                                __cuda_single_ndp0 += task.batch_ndp;
                            }
                            continue;
                        }
                    }
                    ...
                    if (!task.cred_done) {
                        ...
                        const double cred_sec = bgj_epi8_wall_time() - t0;
                        __cuda_cred_time0 += cred_sec;
                        task.cred_done = 1;
                    }

                    const double t0 = bgj_epi8_wall_time();
                    for (long i = 0; i < task.num_bucket; i++) {
                        _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(task.buckets[i], sol_list[owner], goal_norm, &local_profile);
                        _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(task.buckets[i], sol_list[owner], goal_norm, &local_profile);
                        _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(task.buckets[i], sol_list[owner], goal_norm, &local_profile);
                    }
                    __cuda_fallback_time0 += bgj_epi8_wall_time() - t0;
                    __cuda_fallback_bucket0 += (uint64_t)task.num_bucket;
                    __cuda_fallback_ndp0 += task.batch_ndp;
                }
                ...
                local_profile.cuda_batch_call0 += __cuda_batch_call0;
                local_profile.cuda_fallback_bucket0 += __cuda_fallback_bucket0;
            } else if (cuda_bgj2_search0_ordered && bgj_cuda_search_requested() && cuda_bgj2_search0) {
                TIMER_START;
                long __search0_ndp = 0;
                double __cuda_cred_time0 = 0.0;
                double __cuda_single_time0 = 0.0;
                ...
                auto run_single_search0_bucket =
                    [&](bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *bkt) {
                        const long ndp_long = bgj_epi8_search0_profile_ndp(bkt->num_pvec, bkt->num_nvec);
                        const uint64_t ndp = ndp_long > 0 ? (uint64_t)ndp_long : 0;
                        bool cred_done = false;
                        ...
                        if (bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                            double t0 = 0.0;
                            if (cuda_overlap_cred) {
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda_overlap<BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt,
                                                                                                  sol_list[thread],
                                                                                                  goal_norm,
                                                                                                  &local_profile) > 0;
                                __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                                ...
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda<BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt,
                                                                                          sol_list[thread],
                                                                                          goal_norm,
                                                                                          &local_profile) > 0;
                                __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                        ...
                        if (!cuda_done) run_cpu_search0_bucket(bkt, cred_done, ndp);
                        __search0_ndp += ndp_long;
                    };
                ...
                while (nrem_rbucket > 0) {
                    ...
                    if (cuda_batch_capacity > 1 &&
                        bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                        bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *cuda_batch[BGJ2_EPI8_BUCKET0_BATCHSIZE];
                        long cuda_num_bucket = 1;
                        cuda_batch[0] = bkt;

                        while (cuda_num_bucket < (long)cuda_batch_capacity &&
                               cuda_num_bucket < BGJ2_EPI8_BUCKET0_BATCHSIZE &&
                               nrem_rbucket > 0) {
                            if (local_profile.succ_add2 + local_profile.succ_add3 >
                                num_empty + sorted_index - goal_index) {
                                local_profile.report_bucket_not_used(2, nrem_rbucket, 0);
                            ...
                            bucket_epi8_t<BGJ2_EPI8_REUSE_USE_3RED> *next = rbucket[nrem_rbucket - 1];
                            nrem_rbucket--;
                            if (next) cuda_batch[cuda_num_bucket++] = next;
                        }
                        ...
                        if (cuda_num_bucket >= (long)cuda_min_batch) {
                            ...
                            if (!cuda_cred_on_device) {
                                t0 = bgj_epi8_wall_time();
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    _search_cred<BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                }
                                __cuda_cred_time0 += bgj_epi8_wall_time() - t0;
                            }
                            t0 = bgj_epi8_wall_time();
                            const bool cuda_done =
                                _search_bgj1_cuda_batch<BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch,
                                                                                     cuda_num_bucket,
                                                                                     sol_list[thread],
                                                                                     goal_norm,
                                                                                     &local_profile) > 0;
                            ...
                            if (!cuda_done) {
                                ...
                                t0 = bgj_epi8_wall_time();
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                    _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                    _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                }
                                __cuda_fallback_time0 += bgj_epi8_wall_time() - t0;
                                __cuda_fallback_bucket0 += (uint64_t)cuda_num_bucket;
                                __cuda_fallback_ndp0 += batch_ndp;
                        ...
                        for (long i = 0; i < cuda_num_bucket; i++) {
                            run_single_search0_bucket(cuda_batch[i]);
                        }
                        continue;
                    }

                    run_single_search0_bucket(bkt);
                }
            ...
            for (long thread = 0; thread < search_threads; thread++) {
                ...
                while (nrem_rbucket) {
                    ...
                    bool cuda_done = false;
                    #if defined(HAVE_CUDA)
                    if (bgj_cuda_search_requested() && cuda_bgj2_search0) {
                        const int cuda_cred_on_device = bgj_cuda_cred_transform_requested();
                        const int cuda_overlap_cred = !cuda_cred_on_device && bgj_cuda_overlap_cred_requested();
                        if (cuda_batch_capacity > 1 &&
                            bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                            ...
                            if (cuda_num_bucket >= (long)cuda_min_batch) {
                                ...
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda_batch<BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch,
                                                                                                 cuda_num_bucket,
                                                                                                 sol_list[thread],
                                                                                                 goal_norm,
                                                                                                 &local_profile) > 0;
                                ...
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    ...
                                    bool one_cred_done = false;
                                    bool one_cuda_done = false;
                                    if (bgj_epi8_bucket_pair_dots(cuda_batch[i]->num_pvec, cuda_batch[i]->num_nvec) >= cuda_batch_min_dots) {
                                        double t0 = 0.0;
                                        if (cuda_overlap_cred) {
                                            t0 = bgj_epi8_wall_time();
                                            one_cuda_done =
                                                _search_bgj1_cuda_overlap<BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i],
                                                                                                        sol_list[thread],
                                                                                                        goal_norm,
                                                                                                        &local_profile) > 0;
                                            __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                                            ...
                                            t0 = bgj_epi8_wall_time();
                                            one_cuda_done =
                                                _search_bgj1_cuda<BGJ2_EPI8_REUSE_USE_3RED, 1>(cuda_batch[i],
                                                                                               sol_list[thread],
                                                                                               goal_norm,
                                                                                               &local_profile) > 0;
                                            __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                        ...
                        if (bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                            ...
                            if (cuda_overlap_cred) {
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda_overlap<BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt, sol_list[thread], goal_norm, &local_profile) > 0;
                                __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                                cred_done = true;
                            } else {
                                if (!cuda_cred_on_device) {
                                ...
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda<BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt, sol_list[thread], goal_norm, &local_profile) > 0;
                                __cuda_single_time0 += bgj_epi8_wall_time() - t0;
                            }
                            if (cuda_done) {
                                __cuda_single_bucket0++;
                    ...
                    if (!cuda_done) {
                        ...
                        if (!cred_done) {
                            ...
                            _search_cred<BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt, sol_list[thread], goal_norm, &local_profile);
                            #if defined(HAVE_CUDA)
                            if (bgj_cuda_search_requested() && cuda_bgj2_search0) __cuda_cred_time0 += bgj_epi8_wall_time() - t0;
                            #endif
                        }
                        const double t0 = bgj_epi8_wall_time();
                        _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, BGJ2_EPI8_REUSE_USE_3RED, 1>(bkt, sol_list[thread], goal_norm, &local_profile);
            ...
            for (long thread = 0; thread < search_threads; thread++) {
                ...
                double __cuda_cred_time1 = 0.0;
                double __cuda_single_time1 = 0.0;
                double __cuda_batch_time1 = 0.0;
                double __cuda_fallback_time1 = 0.0;
                uint64_t __cuda_single_ndp1 = 0;
                uint64_t __cuda_batch_ndp1 = 0;
                uint64_t __cuda_fallback_ndp1 = 0;
                uint64_t __cuda_single_bucket1 = 0;
                uint64_t __cuda_batch_bucket1 = 0;
                uint64_t __cuda_batch_call1 = 0;
                uint64_t __cuda_fallback_bucket1 = 0;
                while (nrem_bucket1) {
                    ...
                    bool cuda_done = false;
                    #if defined(HAVE_CUDA)
                    if (bgj_cuda_search_requested() && cuda_bgj2_search1) {
                        const int cuda_cred_on_device = bgj_cuda_cred_transform_requested();
                        if (cuda_batch_capacity > 1 &&
                            bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                            bucket_epi8_t<0> *cuda_batch[BGJ2_EPI8_BUCKET1_BATCHSIZE];
                            long cuda_num_bucket = 1;
                            cuda_batch[0] = bkt;

                            pthread_spin_lock(&bucket1_list_lock);
                            while (cuda_num_bucket < (long)cuda_batch_capacity &&
                                   cuda_num_bucket < BGJ2_EPI8_BUCKET1_BATCHSIZE &&
                                   nrem_bucket1 > 0) {
                                if (local_profile.succ_add2 + local_profile.succ_add3 >
                                    num_empty + sorted_index - goal_index) {
                                    local_profile.report_bucket_not_used(2, 0, nrem_bucket1);
                                ...
                                bucket_epi8_t<0> *next = bucket1[nrem_bucket1 - 1];
                                nrem_bucket1--;
                                if (next) cuda_batch[cuda_num_bucket++] = next;
                            }
                            pthread_spin_unlock(&bucket1_list_lock);
                            ...
                            if (cuda_num_bucket >= (long)cuda_min_batch) {
                                ...
                                if (!cuda_cred_on_device) {
                                    t0 = bgj_epi8_wall_time();
                                    for (long i = 0; i < cuda_num_bucket; i++) {
                                        _search_cred<0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                    }
                                    __cuda_cred_time1 += bgj_epi8_wall_time() - t0;
                                }
                                t0 = bgj_epi8_wall_time();
                                cuda_done = _search_bgj1_cuda_batch<0, 1>(cuda_batch,
                                                                          cuda_num_bucket,
                                                                          sol_list[thread],
                                                                          goal_norm,
                                                                          &local_profile) > 0;
                                __cuda_batch_time1 += bgj_epi8_wall_time() - t0;
                                __cuda_batch_call1++;
                                if (!cuda_done) {
                                    if (cuda_cred_on_device) {
                                        t0 = bgj_epi8_wall_time();
                                    ...
                                    t0 = bgj_epi8_wall_time();
                                    for (long i = 0; i < cuda_num_bucket; i++) {
                                        _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                        _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                        _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                    }
                                    __cuda_fallback_time1 += bgj_epi8_wall_time() - t0;
                                    __cuda_fallback_bucket1 += (uint64_t)cuda_num_bucket;
                                    __cuda_fallback_ndp1 += batch_ndp;
                                } else {
                                    __cuda_batch_bucket1 += (uint64_t)cuda_num_bucket;
                                    __cuda_batch_ndp1 += batch_ndp;
                                }
                                __search1_ndp += batch_ndp_long;
                                continue;
                            } else if (cuda_num_bucket > 1) {
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    const long ndp_long = bgj_epi8_search0_profile_ndp(cuda_batch[i]->num_pvec,
                                    ...
                                    if (bgj_epi8_bucket_pair_dots(cuda_batch[i]->num_pvec, cuda_batch[i]->num_nvec) >= cuda_batch_min_dots) {
                                        ...
                                        t0 = bgj_epi8_wall_time();
                                        one_cuda_done =
                                            _search_bgj1_cuda<0, 1>(cuda_batch[i],
                                                                    sol_list[thread],
                                                                    goal_norm,
                                                                    &local_profile) > 0;
                                        __cuda_single_time1 += bgj_epi8_wall_time() - t0;
                                    }
                                    if (!one_cuda_done) {
                                        if (!one_cred_done) {
                                            const double cred_t0 = bgj_epi8_wall_time();
                                            _search_cred<0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                            __cuda_cred_time1 += bgj_epi8_wall_time() - cred_t0;
                                        }
                                        const double t0 = bgj_epi8_wall_time();
                                        _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                        _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                        _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(cuda_batch[i], sol_list[thread], goal_norm, &local_profile);
                                        __cuda_fallback_time1 += bgj_epi8_wall_time() - t0;
                                        __cuda_fallback_bucket1++;
                                        __cuda_fallback_ndp1 += ndp;
                                    } else {
                                        __cuda_single_bucket1++;
                                        __cuda_single_ndp1 += ndp;
                                    }
                                    __search1_ndp += ndp_long;
                                }
                                continue;
                            }
                        }
                        ...
                        if (bgj_epi8_bucket_pair_dots(bkt->num_pvec, bkt->num_nvec) >= cuda_batch_min_dots) {
                            ...
                            t0 = bgj_epi8_wall_time();
                            cuda_done = _search_bgj1_cuda<0, 1>(bkt, sol_list[thread], goal_norm, &local_profile) > 0;
                            __cuda_single_time1 += bgj_epi8_wall_time() - t0;
                            if (cuda_done) {
                                __cuda_single_bucket1++;
                                __cuda_single_ndp1 += ndp;
                    ...
                    if (!cuda_done) {
                        ...
                        if (!cred_done) {
                            ...
                            _search_cred<0, 1>(bkt, sol_list[thread], goal_norm, &local_profile);
                            #if defined(HAVE_CUDA)
                            if (bgj_cuda_search_requested() && cuda_bgj2_search1) __cuda_cred_time1 += bgj_epi8_wall_time() - t0;
                            #endif
                        }
                        const double t0 = bgj_epi8_wall_time();
                        ...
                        if (bgj_cuda_search_requested() && cuda_bgj2_search1) {
                            __cuda_fallback_time1 += bgj_epi8_wall_time() - t0;
                            __cuda_fallback_bucket1++;
                            if (ndp_long > 0) __cuda_fallback_ndp1 += (uint64_t)ndp_long;
                        }
                        #endif
                    }
                    __search1_ndp += bgj_epi8_search0_profile_ndp(bkt->num_pvec, bkt->num_nvec);
                }
                pthread_spin_lock(&local_profile.profile_lock);
                local_profile.search1_ndp += __search1_ndp;
                local_profile.cuda_cred_time1 += __cuda_cred_time1;
                local_profile.cuda_single_time1 += __cuda_single_time1;
                local_profile.cuda_batch_time1 += __cuda_batch_time1;
                local_profile.cuda_fallback_time1 += __cuda_fallback_time1;
                local_profile.cuda_single_ndp1 += __cuda_single_ndp1;
                local_profile.cuda_batch_ndp1 += __cuda_batch_ndp1;
                local_profile.cuda_fallback_ndp1 += __cuda_fallback_ndp1;
                local_profile.cuda_single_bucket1 += __cuda_single_bucket1;
                local_profile.cuda_batch_bucket1 += __cuda_batch_bucket1;
                local_profile.cuda_batch_call1 += __cuda_batch_call1;
                local_profile.cuda_fallback_bucket1 += __cuda_fallback_bucket1;
                pthread_spin_unlock(&local_profile.profile_lock);
            }
            TIMER_END;
...
int Pool_epi8_t<nb>::bgj3_Sieve(long log_level, long lps_auto_adj){
    ...
    while (!sieve_is_over(saturation_radius, saturation_ratio) && !sieving_stucked) {
        ...
        do {
            ...
            pthread_spin_init(&bucket_list_lock, PTHREAD_PROCESS_SHARED);
            #if defined(HAVE_CUDA)
            const uint64_t cuda_bgj3_min_dots = bgj_epi8_env_u64("BGJ_CUDA_BGJ3_MIN_DOTS", bgj_cuda_batch_min_dots());
            const uint64_t cuda_bgj3_search2_staged_min_dots =
                bgj_epi8_env_u64("BGJ_CUDA_BGJ3_SEARCH2_STAGED_MIN_DOTS", 1024ULL * 1024ULL);
            const uint64_t cuda_bgj3_search2_np_fused_min_dots =
                bgj_epi8_env_u64("BGJ_CUDA_BGJ3_SEARCH2_NP_FUSED_MIN_DOTS", 1024ULL * 1024ULL);
            const int cuda_bgj3_search = bgj_epi8_env_flag("BGJ_CUDA_BGJ3_SEARCH", 1);
            const int cuda_bgj3_search0 = cuda_bgj3_search ? bgj_epi8_env_flag("BGJ_CUDA_BGJ3_SEARCH0", 1) : 0;
            const int cuda_bgj3_search1 = cuda_bgj3_search ? bgj_epi8_env_flag("BGJ_CUDA_BGJ3_SEARCH1", 1) : 0;
            const int cuda_bgj3_search2 = cuda_bgj3_search ? bgj_epi8_env_flag("BGJ_CUDA_BGJ3_SEARCH2", 1) : 0;
            const uint32_t cuda_bgj3_batch_capacity = bgj3_epi8_cuda_batch_size();
            uint32_t cuda_bgj3_min_batch = cuda_bgj3_batch_capacity < 4 ? cuda_bgj3_batch_capacity : 4;
            cuda_bgj3_min_batch = (uint32_t)bgj_epi8_env_u64("BGJ_CUDA_BGJ3_MIN_BATCH", cuda_bgj3_min_batch);
            if (cuda_bgj3_min_batch < 1) cuda_bgj3_min_batch = 1;
            if (cuda_bgj3_min_batch > cuda_bgj3_batch_capacity) cuda_bgj3_min_batch = cuda_bgj3_batch_capacity;
            const int cuda_bgj3_search2_staged =
                cuda_bgj3_search2 ? bgj3_epi8_cuda_search2_staged_requested() : 0;
            const int cuda_bgj3_search2_full_fused =
                cuda_bgj3_search2 ? bgj3_epi8_cuda_search2_full_fused_requested() : 0;
            const int cuda_bgj3_search2_np_fused =
                cuda_bgj3_search2 ?
                (bgj3_epi8_cuda_search2_np_fused_requested() ||
                 cuda_bgj3_search2_full_fused) :
                0;
            const int cuda_bgj3_search2_cpu_profile =
                cuda_bgj3_search2 ? bgj3_epi8_cuda_search2_cpu_profile_requested() : 0;
            #else
            const int cuda_bgj3_search0 = 0;
            const int cuda_bgj3_search1 = 0;
            const int cuda_bgj3_search2 = 0;
            const int cuda_bgj3_search2_staged = 0;
            const int cuda_bgj3_search2_np_fused = 0;
            const int cuda_bgj3_search2_full_fused = 0;
            const int cuda_bgj3_search2_cpu_profile = 0;
            const uint64_t cuda_bgj3_search2_staged_min_dots = 0;
            const uint64_t cuda_bgj3_search2_np_fused_min_dots = 0;
            #endif
            #pragma omp parallel for
            for (long thread = 0; thread < num_threads; thread++) {
                double cuda2_cpu_cred_time = 0.0;
                double cuda2_cpu_np_time = 0.0;
                double cuda2_cpu_pp_time = 0.0;
                double cuda2_cpu_nn_time = 0.0;
                uint64_t cuda2_cpu_bucket = 0;
                uint64_t cuda2_cpu_ndp = 0;
                uint64_t cuda2_reason_min_dot_bucket = 0;
                uint64_t cuda2_reason_min_dot_ndp = 0;
                uint64_t cuda2_reason_min_batch_bucket = 0;
                uint64_t cuda2_reason_min_batch_ndp = 0;
                uint64_t cuda2_reason_batch_fail_bucket = 0;
                uint64_t cuda2_reason_batch_fail_ndp = 0;
                uint64_t cuda2_reason_consume_fail_bucket = 0;
                uint64_t cuda2_reason_consume_fail_ndp = 0;
                uint64_t cuda2_overflow_bucket = 0;
                uint64_t cuda2_overflow_ndp = 0;
                auto bgj3_search_bucket = [&](auto *search_bkt,
                                              auto record_tag,
                                              sol_list_epi8_t *sol,
                                              int cuda_enabled,
                                              long &search_ndp,
                                              double &cuda_cred_time,
                                              double &cuda_single_time,
                                              double &cuda_fallback_time,
                                              uint64_t &cuda_single_ndp,
                                              uint64_t &cuda_fallback_ndp,
                                              uint64_t &cuda_single_bucket,
                                              uint64_t &cuda_fallback_bucket,
                                              int profile_search2_cpu) {
                    constexpr bool record_dp = decltype(record_tag)::value;
                    const long ndp_long = bgj_epi8_search0_profile_ndp(search_bkt->num_pvec, search_bkt->num_nvec);
                    const uint64_t ndp = ndp_long > 0 ? (uint64_t)ndp_long : 0;
                    bool cred_done = false;
                    bool cuda_done = false;
                    double fallback_cred_time = 0.0;
                    #if defined(HAVE_CUDA)
                    if (bgj_cuda_search_requested() && cuda_enabled &&
                        bgj_epi8_bucket_pair_dots(search_bkt->num_pvec, search_bkt->num_nvec) >= cuda_bgj3_min_dots) {
                        const int cuda_cred_on_device = record_dp && bgj_cuda_cred_transform_requested();
                        const int cuda_overlap_cred = record_dp && !cuda_cred_on_device && bgj_cuda_overlap_cred_requested();
                        double t0 = 0.0;
                        if (cuda_overlap_cred) {
                            t0 = bgj_epi8_wall_time();
                            cuda_done = _search_bgj1_cuda_overlap<record_dp, 1>(search_bkt, sol, goal_norm, &local_profile) > 0;
                            cuda_single_time += bgj_epi8_wall_time() - t0;
                            cred_done = true;
                        } else {
                            if (!cuda_cred_on_device) {
                                t0 = bgj_epi8_wall_time();
                                _search_cred<record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                                const double elapsed = bgj_epi8_wall_time() - t0;
                                cuda_cred_time += elapsed;
                                fallback_cred_time += elapsed;
                                cred_done = true;
                            }
                            t0 = bgj_epi8_wall_time();
                            cuda_done = _search_bgj1_cuda<record_dp, 1>(search_bkt, sol, goal_norm, &local_profile) > 0;
                            cuda_single_time += bgj_epi8_wall_time() - t0;
                        }
                        if (cuda_done) {
                            cuda_single_bucket++;
                    ...
                    if (!cuda_done) {
                        if (!cred_done) {
                            ...
                            _search_cred<record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                            const double elapsed = bgj_epi8_wall_time() - t0;
                            if (profile_search2_cpu) cuda2_cpu_cred_time += elapsed;
                            #if defined(HAVE_CUDA)
                            if (bgj_cuda_search_requested() && cuda_enabled) cuda_cred_time += elapsed;
                            #endif
                        } else if (profile_search2_cpu) {
                            cuda2_cpu_cred_time += fallback_cred_time;
                        }
                        const double t0 = bgj_epi8_wall_time();
                        if (profile_search2_cpu) {
                            double phase_t0 = bgj_epi8_wall_time();
                            _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                            cuda2_cpu_np_time += bgj_epi8_wall_time() - phase_t0;
                            phase_t0 = bgj_epi8_wall_time();
                            _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                            cuda2_cpu_pp_time += bgj_epi8_wall_time() - phase_t0;
                            phase_t0 = bgj_epi8_wall_time();
                            _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                            cuda2_cpu_nn_time += bgj_epi8_wall_time() - phase_t0;
                            cuda2_cpu_bucket++;
                            cuda2_cpu_ndp += ndp;
                        } else {
                            _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, record_dp, 1>(search_bkt, sol, goal_norm, &local_profile);
                        ...
                        #if defined(HAVE_CUDA)
                        if (bgj_cuda_search_requested() && cuda_enabled) {
                            cuda_fallback_time += bgj_epi8_wall_time() - t0;
                            cuda_fallback_bucket++;
                            cuda_fallback_ndp += ndp;
                        }
                        #endif
                    }
                    search_ndp += ndp_long;
                };
                auto bgj3_flush_search2_batch = [&](bucket_epi8_t<0> **cuda_batch,
                                                    long &cuda_num_bucket,
                                                    sol_list_epi8_t *sol,
                                                    long &search_ndp,
                                                    double &cuda_cred_time,
                                                    double &cuda_single_time,
                                                    double &cuda_batch_time,
                                                    double &cuda_fallback_time,
                                                    uint64_t &cuda_single_ndp,
                                                    uint64_t &cuda_batch_ndp,
                                                    uint64_t &cuda_fallback_ndp,
                                                    uint64_t &cuda_single_bucket,
                                                    uint64_t &cuda_batch_bucket,
                                                    uint64_t &cuda_batch_call,
                                                    uint64_t &cuda_fallback_bucket) {
                    if (cuda_num_bucket <= 0) return;
                    long batch_ndp_long = 0;
                    uint64_t batch_ndp = 0;
                    uint64_t bucket_ndp[BGJ3_EPI8_BUCKET2_BATCHSIZE] = {};
                    long bucket_ndp_long[BGJ3_EPI8_BUCKET2_BATCHSIZE] = {};
                    for (long i = 0; i < cuda_num_bucket; i++) {
                        const long ndp = bgj_epi8_search0_profile_ndp(cuda_batch[i]->num_pvec,
                                                                      cuda_batch[i]->num_nvec);
                        bucket_ndp_long[i] = ndp;
                        batch_ndp_long += ndp;
                        if (ndp > 0) {
                            bucket_ndp[i] = (uint64_t)ndp;
                            batch_ndp += (uint64_t)ndp;
                        }
                    }
                    auto run_cpu_search2_after_cred_bucket =
                        [&](bucket_epi8_t<0> *bkt, long ndp_long, uint64_t ndp) {
                            const double t0 = bgj_epi8_wall_time();
                            if (cuda_bgj3_search2_cpu_profile) {
                                double phase_t0 = bgj_epi8_wall_time();
                                _search_np<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(bkt, sol, goal_norm, &local_profile);
                                cuda2_cpu_np_time += bgj_epi8_wall_time() - phase_t0;
                                phase_t0 = bgj_epi8_wall_time();
                                _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(bkt, sol, goal_norm, &local_profile);
                                cuda2_cpu_pp_time += bgj_epi8_wall_time() - phase_t0;
                                phase_t0 = bgj_epi8_wall_time();
                                _search_nn<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(bkt, sol, goal_norm, &local_profile);
                                cuda2_cpu_nn_time += bgj_epi8_wall_time() - phase_t0;
                                cuda2_cpu_bucket++;
                                cuda2_cpu_ndp += ndp;
                            } else {
                    ...
                    auto run_cpu_search2_bucket =
                        [&](bucket_epi8_t<0> *bkt, long ndp_long, uint64_t ndp) {
                            if (cuda_bgj3_search2_cpu_profile) {
                                const double phase_t0 = bgj_epi8_wall_time();
                                _search_cred<0, 1>(bkt, sol, goal_norm, &local_profile);
                                cuda2_cpu_cred_time += bgj_epi8_wall_time() - phase_t0;
                            } else {
                                _search_cred<0, 1>(bkt, sol, goal_norm, &local_profile);
                            }
                            run_cpu_search2_after_cred_bucket(bkt, ndp_long, ndp);
                        };
                    auto run_cpu_search2_tail_bucket =
                        [&](bucket_epi8_t<0> *bkt, long ndp_long) {
                            if (cuda_bgj3_search2_cpu_profile) {
                                double phase_t0 = bgj_epi8_wall_time();
                                _search_pp<SEARCH_L2_BLOCK, SEARCH_L1_BLOCK, 0, 1>(bkt, sol, goal_norm, &local_profile);
                                cuda2_cpu_pp_time += bgj_epi8_wall_time() - phase_t0;
                    ...
                    #if defined(HAVE_CUDA)
                    static thread_local std::vector<bgj_cuda_result_t> staged_results;
                    static thread_local std::vector<bgj_cuda_result_t *> staged_result_ptrs;
                    static thread_local std::vector<uint32_t> staged_result_counts;
                    static thread_local std::vector<int> staged_overflows;
                    const uint64_t staged_batch_min_dots =
                        cuda_bgj3_search2_staged ? cuda_bgj3_search2_staged_min_dots :
                                                    cuda_bgj3_min_dots;
                    if (bgj_cuda_search_requested() &&
                        cuda_bgj3_search2 &&
                        cuda_bgj3_search2_np_fused &&
                        cuda_bgj3_batch_capacity > 1 &&
                        cuda_num_bucket > 1 &&
                        batch_ndp >= cuda_bgj3_search2_np_fused_min_dots) {
                        const uint32_t result_capacity =
                            bgj3_epi8_cuda_search2_np_fused_result_capacity();
                        int storage_ok = result_capacity != 0;
                        try {
                            staged_results.resize((size_t)cuda_num_bucket * (size_t)result_capacity);
                            staged_result_ptrs.resize((size_t)cuda_num_bucket);
                            staged_result_counts.assign((size_t)cuda_num_bucket, 0);
                            staged_overflows.assign((size_t)cuda_num_bucket, 0);
                        } catch (...) {
                            staged_results.clear();
                            staged_result_ptrs.clear();
                            staged_result_counts.clear();
                            staged_overflows.clear();
                            storage_ok = 0;
                        }
                        if (storage_ok) {
                            const double cred_t0 = bgj_epi8_wall_time();
                            for (long i = 0; i < cuda_num_bucket; i++) {
                                _search_cred<0, 1>(cuda_batch[i], sol, goal_norm, &local_profile);
                            }
                            const double cred_elapsed = bgj_epi8_wall_time() - cred_t0;
                            cuda_cred_time += cred_elapsed;
                            if (cuda_bgj3_search2_cpu_profile) {
                                cuda2_cpu_cred_time += cred_elapsed;
                            }
                        }
                        double t0 = bgj_epi8_wall_time();
                        const int cuda_done =
                            storage_ok ?
                            (_search_bgj1_cuda_np_batch_collect<0, 1>(
                                 cuda_batch,
                                 cuda_num_bucket,
                                 goal_norm,
                                 cuda_bgj3_search2_full_fused ? 1 : 0,
                                 staged_results.data(),
                                 result_capacity,
                                 staged_result_ptrs.data(),
                                 staged_result_counts.data(),
                                 staged_overflows.data()) > 0) :
                            0;
                        cuda_batch_time += bgj_epi8_wall_time() - t0;
                        cuda_batch_call++;
                        if (cuda_done) {
                            for (long i = 0; i < cuda_num_bucket; i++) {
                                if (staged_overflows[(size_t)i]) {
                                    cuda2_overflow_bucket++;
                                    cuda2_overflow_ndp += bucket_ndp[i];
                                    run_cpu_search2_after_cred_bucket(cuda_batch[i],
                                                                      bucket_ndp_long[i],
                                                                      bucket_ndp[i]);
                                    continue;
                                }
                                if (_consume_bgj1_cuda_results<0, 1>(
                                        cuda_batch[i],
                                        sol,
                                        &local_profile,
                                        staged_result_ptrs[(size_t)i],
                                        staged_result_counts[(size_t)i])) {
                                    if (cuda_bgj3_search2_full_fused) {
                                        search_ndp += bucket_ndp_long[i];
                                    } else {
                                        run_cpu_search2_tail_bucket(cuda_batch[i],
                                                                   bucket_ndp_long[i]);
                                    }
                                    cuda_batch_bucket++;
                                    cuda_batch_ndp += bucket_ndp[i];
                                } else {
                                    cuda2_reason_consume_fail_bucket++;
                                    cuda2_reason_consume_fail_ndp += bucket_ndp[i];
                                    run_cpu_search2_after_cred_bucket(cuda_batch[i],
                                                                      bucket_ndp_long[i],
                                                                      bucket_ndp[i]);
                                }
                        ...
                        cuda2_reason_batch_fail_bucket += (uint64_t)cuda_num_bucket;
                        cuda2_reason_batch_fail_ndp += batch_ndp;
                        if (storage_ok) {
                            for (long i = 0; i < cuda_num_bucket; i++) {
                                run_cpu_search2_after_cred_bucket(cuda_batch[i],
                                                                  bucket_ndp_long[i],
                    ...
                    if (bgj_cuda_search_requested() && cuda_bgj3_search2 &&
                        !cuda_bgj3_search2_np_fused &&
                        cuda_bgj3_batch_capacity > 1 &&
                        ((!cuda_bgj3_search2_staged &&
                          cuda_num_bucket >= (long)cuda_bgj3_min_batch) ||
                         (cuda_bgj3_search2_staged &&
                          cuda_num_bucket > 1 &&
                          batch_ndp >= staged_batch_min_dots))) {
                        const int cuda_cred_on_device = bgj_cuda_cred_transform_requested();
                        ...
                        if (!cuda_cred_on_device) {
                            ...
                            cuda_cred_time += bgj_epi8_wall_time() - t0;
                        }

                        t0 = bgj_epi8_wall_time();
                        int cuda_done = 0;
                        if (cuda_bgj3_search2_staged) {
                            const uint32_t result_capacity = bgj_cuda_result_capacity();
                            int storage_ok = result_capacity != 0;
                            try {
                                staged_results.resize((size_t)cuda_num_bucket * (size_t)result_capacity);
                                staged_result_ptrs.resize((size_t)cuda_num_bucket);
                            ...
                            if (storage_ok) {
                                cuda_done =
                                    _search_bgj1_cuda_batch_collect<0, 1>(
                                        cuda_batch,
                                        cuda_num_bucket,
                                        goal_norm,
                                        staged_results.data(),
                            ...
                            if (cuda_bgj3_search2_staged) {
                                for (long i = 0; i < cuda_num_bucket; i++) {
                                    ...
                                    if (_consume_bgj1_cuda_results<0, 1>(
                                        ...
                                        cuda2_reason_consume_fail_bucket++;
                                        cuda2_reason_consume_fail_ndp += bucket_ndp[i];
                                        run_cpu_search2_bucket(cuda_batch[i],
                                                               bucket_ndp_long[i],
                                                               bucket_ndp[i]);
                                    }
                                }
                            } else {
                                cuda_batch_bucket += (uint64_t)cuda_num_bucket;
                                cuda_batch_ndp += batch_ndp;
                                search_ndp += batch_ndp_long;
                            }
                            cuda_num_bucket = 0;
                            return;
                        }
                        ...
                        cuda2_reason_batch_fail_ndp += batch_ndp;
                        for (long i = 0; i < cuda_num_bucket; i++) {
                            run_cpu_search2_bucket(cuda_batch[i], bucket_ndp_long[i], bucket_ndp[i]);
                        }
                        cuda_num_bucket = 0;
                        return;
                    }
                    if (bgj_cuda_search_requested() &&
                        ...
                        if (!cuda_bgj3_search2_staged &&
                            !cuda_bgj3_search2_np_fused &&
                            cuda_num_bucket < (long)cuda_bgj3_min_batch) {
                            cuda2_reason_min_batch_bucket += (uint64_t)cuda_num_bucket;
                            cuda2_reason_min_batch_ndp += batch_ndp;
                        } else if (cuda_bgj3_search2_staged &&
                                   batch_ndp < staged_batch_min_dots) {
                            cuda2_reason_min_batch_bucket += (uint64_t)cuda_num_bucket;
                            cuda2_reason_min_batch_ndp += batch_ndp;
                        } else if (cuda_bgj3_search2_np_fused &&
                                   batch_ndp < cuda_bgj3_search2_np_fused_min_dots) {
                            cuda2_reason_min_batch_bucket += (uint64_t)cuda_num_bucket;
                            cuda2_reason_min_batch_ndp += batch_ndp;
                        }
                    ...
                    for (long i = 0; i < cuda_num_bucket; i++) {
                        ...
                        if (bgj_cuda_search_requested() &&
                            cuda_bgj3_search2 &&
                            bgj_epi8_bucket_pair_dots(cuda_batch[i]->num_pvec,
                                                      cuda_batch[i]->num_nvec) < cuda_bgj3_min_dots) {
                            cuda2_reason_min_dot_bucket++;
                            cuda2_reason_min_dot_ndp += bucket_ndp[i];
                        }
                        #endif
                        bgj3_search_bucket(cuda_batch[i],
                ...
                for (;;) {
                    ...
                    if (type == -1) continue;
                    if (type == 0) {
                        const double __search0_t0 = bgj_epi8_wall_time();
                        long __search0_ndp = 0;
                        double __cuda_cred_time0 = 0.0;
                        double __cuda_single_time0 = 0.0;
                        double __cuda_fallback_time0 = 0.0;
                        ...
                        uint64_t __cuda_single_bucket0 = 0;
                        uint64_t __cuda_fallback_bucket0 = 0;
                        bgj3_search_bucket(bkt3,
                                           std::integral_constant<bool, BGJ3_EPI8_REUSE0_USE_3RED>(),
                                           sol_list[thread],
                                           cuda_bgj3_search0,
                                           __search0_ndp,
                                           __cuda_cred_time0,
                                           __cuda_single_time0,
                                           __cuda_fallback_time0,
                                           __cuda_single_ndp0,
                                           __cuda_fallback_ndp0,
                                           __cuda_single_bucket0,
                                           __cuda_fallback_bucket0,
                                           0);
                        pthread_spin_lock(&local_profile.profile_lock);
                        local_profile.search0_time += bgj_epi8_wall_time() - __search0_t0;
                        local_profile.search0_ndp += __search0_ndp;
                        local_profile.cuda_cred_time0 += __cuda_cred_time0;
                        local_profile.cuda_single_time0 += __cuda_single_time0;
                        local_profile.cuda_fallback_time0 += __cuda_fallback_time0;
                        ...
                        double __cuda_cred_time2 = 0.0;
                        double __cuda_single_time2 = 0.0;
                        double __cuda_batch_time2 = 0.0;
                        double __cuda_fallback_time2 = 0.0;
                        uint64_t __cuda_single_ndp2 = 0;
                        uint64_t __cuda_batch_ndp2 = 0;
                        uint64_t __cuda_fallback_ndp2 = 0;
                        uint64_t __cuda_single_bucket2 = 0;
                        uint64_t __cuda_batch_bucket2 = 0;
                        uint64_t __cuda_batch_call2 = 0;
                        uint64_t __cuda_fallback_bucket2 = 0;
                        ...
                        uint64_t __cuda_single_bucket1 = 0;
                        uint64_t __cuda_fallback_bucket1 = 0;
                        const double __search1_t0 = bgj_epi8_wall_time();
                        bgj3_search_bucket(bkt2,
                                           std::integral_constant<bool, false>(),
                                           sol_list[thread],
                                           cuda_bgj3_search1,
                                           __search1_ndp,
                                           __cuda_cred_time1,
                                           __cuda_single_time1,
                                           __cuda_fallback_time1,
                                           __cuda_single_ndp1,
                                           __cuda_fallback_ndp1,
                                           __cuda_single_bucket1,
                                           __cuda_fallback_bucket1,
                                           0);
                        __search1_time += bgj_epi8_wall_time() - __search1_t0;
                        delete bkt2;
                        ...
                        bucket_epi8_t<0> *cuda_search2_batch[BGJ3_EPI8_BUCKET2_BATCHSIZE];
                        long cuda_search2_num_bucket = 0;
                        const double __search2_t0 = bgj_epi8_wall_time();
                        for (long i = 0; i < BGJ3_EPI8_BUCKET2_BATCHSIZE; i++) {
                            bucket_epi8_t<0> *_bkt = bucket2[thread * BGJ3_EPI8_BUCKET2_BATCHSIZE + i];
                            if (_bkt == NULL) continue;
                            bool cuda_batchable = false;
                            #if defined(HAVE_CUDA)
                            const uint64_t search2_work =
                                bgj_epi8_bucket_pair_dots(_bkt->num_pvec, _bkt->num_nvec);
                            cuda_batchable = bgj_cuda_search_requested() &&
                                             cuda_bgj3_search2 &&
                                             cuda_bgj3_batch_capacity > 1 &&
                                             ((cuda_bgj3_search2_staged ||
                                               cuda_bgj3_search2_np_fused) ?
                                              search2_work > 0 :
                                              search2_work >= cuda_bgj3_min_dots);
                            #endif
                            if (cuda_batchable) {
                                cuda_search2_batch[cuda_search2_num_bucket++] = _bkt;
                                #if defined(HAVE_CUDA)
                                if (cuda_search2_num_bucket >= (long)cuda_bgj3_batch_capacity) {
                                    bgj3_flush_search2_batch(cuda_search2_batch,
                                                             cuda_search2_num_bucket,
                                                             sol_list[thread],
                                                             __search2_ndp,
                                                             __cuda_cred_time2,
                                                             __cuda_single_time2,
                                                             __cuda_batch_time2,
                                                             __cuda_fallback_time2,
                                                             __cuda_single_ndp2,
                                                             __cuda_batch_ndp2,
                                                             __cuda_fallback_ndp2,
                                                             __cuda_single_bucket2,
                                                             __cuda_batch_bucket2,
                                                             __cuda_batch_call2,
                                                             __cuda_fallback_bucket2);
                                }
                                #endif
                            } else {
                                #if defined(HAVE_CUDA)
                                if (bgj_cuda_search_requested() &&
                                    ...
                                    search2_work < cuda_bgj3_min_dots) {
                                    cuda2_reason_min_dot_bucket++;
                                    cuda2_reason_min_dot_ndp += search2_work;
                                }
                                #endif
                                bgj3_search_bucket(_bkt,
                                                   std::integral_constant<bool, false>(),
                                                   sol_list[thread],
                                                   cuda_bgj3_search2,
                                                   __search2_ndp,
                        ...
                        bgj3_flush_search2_batch(cuda_search2_batch,
                                                 ...
                                                 __cuda_batch_call2,
                                                 __cuda_fallback_bucket2);
                        __search2_time += bgj_epi8_wall_time() - __search2_t0;
                        ...
                        local_profile.cuda_single_bucket1 += __cuda_single_bucket1;
                        local_profile.cuda_fallback_bucket1 += __cuda_fallback_bucket1;
                        local_profile.cuda_cred_time2 += __cuda_cred_time2;
                        local_profile.cuda_single_time2 += __cuda_single_time2;
                        local_profile.cuda_batch_time2 += __cuda_batch_time2;
                        local_profile.cuda_fallback_time2 += __cuda_fallback_time2;
                        local_profile.cuda_single_ndp2 += __cuda_single_ndp2;
                        local_profile.cuda_batch_ndp2 += __cuda_batch_ndp2;
                        local_profile.cuda_fallback_ndp2 += __cuda_fallback_ndp2;
                        local_profile.cuda_single_bucket2 += __cuda_single_bucket2;
                        local_profile.cuda_batch_bucket2 += __cuda_batch_bucket2;
                        local_profile.cuda_batch_call2 += __cuda_batch_call2;
                        local_profile.cuda_fallback_bucket2 += __cuda_fallback_bucket2;
                        pthread_spin_unlock(&local_profile.profile_lock);
                        delete bkt;
                    }
                    if (local_profile.succ_add2 + local_profile.succ_add3 > num_empty + sorted_index - goal_index) {
                ...
                pthread_spin_lock(&local_profile.profile_lock);
                local_profile.cuda2_reason_min_dot_bucket += cuda2_reason_min_dot_bucket;
                local_profile.cuda2_reason_min_dot_ndp += cuda2_reason_min_dot_ndp;
                local_profile.cuda2_reason_min_batch_bucket += cuda2_reason_min_batch_bucket;
                local_profile.cuda2_reason_min_batch_ndp += cuda2_reason_min_batch_ndp;
                local_profile.cuda2_reason_batch_fail_bucket += cuda2_reason_batch_fail_bucket;
                local_profile.cuda2_reason_batch_fail_ndp += cuda2_reason_batch_fail_ndp;
                local_profile.cuda2_reason_consume_fail_bucket += cuda2_reason_consume_fail_bucket;
                local_profile.cuda2_reason_consume_fail_ndp += cuda2_reason_consume_fail_ndp;
                local_profile.cuda2_overflow_bucket += cuda2_overflow_bucket;
                local_profile.cuda2_overflow_ndp += cuda2_overflow_ndp;
                local_profile.cuda2_cpu_bucket += cuda2_cpu_bucket;
                local_profile.cuda2_cpu_ndp += cuda2_cpu_ndp;
                local_profile.cuda2_cpu_cred_time += cuda2_cpu_cred_time;
                local_profile.cuda2_cpu_np_time += cuda2_cpu_np_time;
                local_profile.cuda2_cpu_pp_time += cuda2_cpu_pp_time;
                local_profile.cuda2_cpu_nn_time += cuda2_cpu_nn_time;
                pthread_spin_unlock(&local_profile.profile_lock);
            }
